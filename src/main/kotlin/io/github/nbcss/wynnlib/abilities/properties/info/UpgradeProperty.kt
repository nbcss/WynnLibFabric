...
class UpgradeProperty(ability: Ability, data: JsonElement): AbilityProperty(ability) {
    ...
    fun getUpgradeSpell(): SpellSlot? = spell

    private fun getUpgradeEntry(container: EntryContainer): PropertyEntry? {
        if (spell != null){
    ...
    override fun updateEntries(container: EntryContainer) {
        getUpgradeEntry(container)?.let { entry ->
            for (property in getAbility().getProperties()) {
                if (property is UpgradeableProperty){
